...
Production deployment directory uses one owner-only `.env` as the canonical Compose and
application configuration. Build it once from the tracked templates:
...
`deploy/compose.production.env` fixes project name, production base file, Xray overlay and
canonical service env file. Therefore every routine command is executed directly from the
deployment directory, without `-p`, `-f`, `--env-file` or `--profile`:
...
Set `COMPOSE_PROFILES=telegram` in `.env` only when Bot/Stars variables contain real values.
Without it the radio stack runs without the optional Telegram bot service.

The Xray secret has exact top-level shape `{ "outbounds": [...] }`, contains exactly one
outbound tagged `proxy`, and contains no `direct`/`freedom` protocol. VLESS/REALITY, Trojan,
SOCKS and non-direct helper outbounds remain Xray-native JSON. Infrastructure-owned
`inbounds`, `routing`, and logging stay in `deploy/xray/base.json`; endpoint credentials stay
only in `.secrets/xray-outbound.json`.

Invalid secret exits before redirect rules are installed. `docker compose logs xray-proxy`
shows validation failures; `docker compose ps` shows tunnel health. Terminal
`RTMP output failed` requires restoring the tunnel and issuing admin **Restart**.

To switch to direct Telegram egress, set `COMPOSE_FILE=compose.prod.yaml` and direct RTMP
values in `.env`, then apply the topology:

```sh
docker compose up -d --wait --wait-timeout 180 --remove-orphans
```

Development `compose.yaml` and its local `rtmp-sink` do not use the Xray overlay.
