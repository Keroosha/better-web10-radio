...
- `Web10.Radio.Database` — PostgreSQL options, FluentMigrator migrations through `202607130002`, `pg_trgm` search indexes, ADO.NET transaction/session helpers и repositories.
...
- Local and S3 library scans accept external FLAC+CUE libraries. One FLAC source becomes individually searchable/queueable CUE tracks; stale `FILE "...wav"` declarations resolve to the matching `.flac` stem, and stream-node playback decodes only the selected CUE window.
...
## Production Xray outbound secret

Опциональный production overlay `compose.xray.yaml` принимает один deploy-local Xray client
secret. Скопируйте существующий VLESS/REALITY, Trojan или SOCKS client outbound object в
gitignored `.secrets/xray-outbound.json` и назначьте выбранному egress exact tag `proxy`:
...
Top-level object содержит только non-empty `outbounds`. Ровно один outbound имеет
`tag: "proxy"`; protocol `direct`/`freedom` запрещён в любом регистре. Дополнительные
non-direct helper outbounds допустимы. Infrastructure-owned `inbounds`, `routing` и logging
остаются в `deploy/xray/base.json` и не переносятся в secret. Endpoint credentials и
protocol-specific application variables в repository не сохраняются. Файл должен оставаться
owner-only (`chmod 0600 .secrets/xray-outbound.json`).

Подготовка и proxy-on:

```sh
cp .env.xray.example .env.xray
mkdir -p .secrets
chmod 0700 .secrets
# Заполните .secrets/xray-outbound.json реальным client outbound.
chmod 0600 .secrets/xray-outbound.json

docker compose -p web10-radio-prod \
  --env-file .env.prod --env-file .env.xray \
  -f compose.prod.yaml -f compose.xray.yaml \
  --profile telegram up -d --wait --wait-timeout 180 --remove-orphans
```

После переезда на сеть с прямым Telegram egress выключение proxy — omission overlay с тем же
project name. `--remove-orphans` удалит `xray-proxy`, а Telegram и stream-node будут
пересозданы в direct topology:
...
Invalid secret завершается до установки redirect rules; смотрите
`docker compose ... logs xray-proxy`. `unhealthy` означает, что TLS GET к
`api.telegram.org` через remote tunnel не прошёл. Проверка реального Bot API path:
`docker compose ... exec telegram dotnet Web10.Radio.Telegram.dll --health-check
http://127.0.0.1:8080/health/ready`. Для RTMP acceptance нужен наблюдаемый эфир в Telegram;
terminal `RTMP output failed` в stream-node logs требует восстановления tunnel и admin
Restart. Подробный runbook — `docs/getting_started.md`.

Development `compose.yaml` и его `rtmp-sink` не используют `compose.xray.yaml`.
...
dotnet run --project src/stream-node/Web10.Radio.StreamNode.Tools/Web10.Radio.StreamNode.Tools.fsproj -- smoke-backend --mode restart-live --base-url http://localhost:8080 --rtmp-stat-url http://localhost:8091/stat --username admin --password admin-password --timeout-seconds 60
dotnet run --project src/stream-node/Web10.Radio.StreamNode.Tools/Web10.Radio.StreamNode.Tools.fsproj -- smoke-backend --mode flac-cue --base-url http://localhost:8080 --rtmp-stat-url http://localhost:8091/stat --username admin --password admin-password --timeout-seconds 60
...
Ожидаемый migration tail: `202607130001`, `202607130002`. Для ручной cleanup вне smoke block: `docker compose down --volumes --remove-orphans`.
