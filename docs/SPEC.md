...
| `GET` | `/api/v0/stream-node/playback/{queueItemId}/media` | Stream the assigned track's audio bytes for the current fenced item. |
...
- `GET /api/v0/stream-node/playback/{queueItemId}/media` streams the assigned track's audio bytes (range-enabled) for a currently `Claimed`/`Playing` fenced item, so the stream-node needs no shared filesystem access to the materialized cache. It returns `404 playback.media_not_found` when the item is not active or its cached file is unavailable.
